...
def find_afterparties():
    ...
    # these are the form questions on the browser and we are grabbing the
    # users responses
    ...
    if location and distance and measurement:
        ...
        # q, sort_by, location.within, location.address, token 
        # came from the eventbrite API and the values are the form 
        # responses filled by the user on the browser
        payload = {'q': query,
                   'sort_by': sort,
                   'location.within': distance,
                   'location.address': location,
                   'token': os.environ['EVENTBRITE_TOKEN']} #so that our token
                   #is hidden when pushing to github
        print(payload)
        #url field in eventbright is a string type
        url = 'https://www.eventbriteapi.com/v3/events/search'

        #this creates the long url with the users form responses
        #so they're searching for afterparties in a specific location
        #at a specific distance
        response = requests.get(url, params=payload)

        #this is will show the keys
        print(response.headers)

        #this will show the content/values
        print(response.content)

        #converts the data from JSON to a python dictionary
        data = response.json()

        #[data] are dictionary key value pairs that we will loop through
        #in afterparties.html, use .items and append the values to an 
        #empty list
        events = [data]
...
def create_eventbrite_event():
    ...
    url = '''https://www.eventbriteapi.com/v3/organizations/organization_id/
             events/'''
    ...
    headers = {'Authorization': 'Bearer' + os.eviron['EVENTBRITE_TOKEN']}

    response = requests.post(url, data=payload, headers=headers)

    json = response.json()
    event = json.event_id # ???
    event_url = '''https://www.eventbriteapi.com/v3/organizations/
                organization_id/events/''' + str(event) + '/'
